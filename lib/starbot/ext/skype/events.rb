  ...
  class Events
    class << self
      ...
      def on(event)
        ...
        when :users_received
          Skype::Api.instance.on_notification("USERS (.*)$") do |user_ids|
      ...
      # @example
      #   Skype::Events.watch_until(:buddystatus, 'simulacrejp') do |user_id, status|
      #     # ...
      #     done? ? true : false
      #   end #  |user_id, status|
      ...
      def initialize_listeners
        ...
        umutex = Mutex.new
        Skype::Events.on(:users_received) do |users|
          umutex.synchronize { users.each{|u| Skype::User.users.push(u) unless Skype::Users.users.include?(u) } }
        end #  |users|
        
        Skype::Api.instance.on_notification("USER (.*) BUDDYSTATUS (.*)") do |user_id, bstatus|
