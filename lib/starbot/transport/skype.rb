  ...
  module Transport
    class Skype
      ...
      def initialize(bot, defroom, opts = {})
        ...
        watch_rooms
        watch_msgs
        ::Skype::Api.instance.verbose = opts[:verbose]        
        ::Skype.attach
      ...
      def sayto_contact(contact, msg)
        ...
        unless msg.nil? || msg.empty?
          if contact.is_a?(String)
            ::Skype.chat(contact).send_message(msg.force_encoding('BINARY'))
          else
            rms = @bot.room_list.find_with_only_user(contact)
            if !rms[0].nil?
              ::Skype.chat(rms[0].id).send_message(msg.force_encoding('BINARY'))
            else
              ::Skype::Chat.create(contact.id) do |room|
                ::Skype.chat(room.id).send_message(msg.force_encoding('BINARY'))
              end
            end # rms[0].nil?
      ...
      def sayto_room(room, msg)
        ::Skype.chat(room.id).send_message(msg.force_encoding('BINARY')) unless msg.nil? || msg.empty?
        nil
      ...
      def leave_room(room_id)
        ::Skype::Chat.new(room_id).leave
      end # leave_room(room_id)
      
      def invite_room(rid, user, *users, &blk)
        blk.nil? ? ::Skype::Chat.new(rid).invite(user, *users) : ::Skype::Chat.new(rid).invite("#{user}", *users, &blk)
      end # invite_room(rid, user, *users, &blk)
      
      def start_room(name, uid, *uids)
        ::Skype::Chat.create(uid, *uids) do |cht|
      ...
      def when_authorized(user_id)
        user = ::Skype::User.users.find{|u| u.id == user_id} || ::Skype::User.new(user_id)
      ...
      def refresh_contacts
        ::Skype::User.friends do |friends|
          friends.each do |friend|
            friend.get_props do |props|
              @bot.contact_list.create(friend.id, props[:fullname]).tap do |contact|
                case props[:status]
                when ::Skype::User::BuddyStatus::NEVERINLIST
                  contact.status = :notinlist
                when ::Skype::User::BuddyStatus::DELFROMLIST
                when ::Skype::User::BuddyStatus::PENDINGAUTH
                  contact.status = :waiting
                when ::Skype::User::BuddyStatus::INLIST
      ...
      def watch_rooms
        ::Skype::Api.instance.on_notification("CHAT (\#.*) STATUS (DIALOG|MULTI_SUBSCRIBED)$") do |name,type|
          @logger.debug("+++++++ new chat #{name} ++++++")
          ::Skype::Chat.new(name).tap do |chat|
